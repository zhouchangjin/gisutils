...
public class TiffUtils {
    ...
    public static BufferedImage CalcSlop2Band(RenderedImage image, double cellsize){
        ...
        for(int i=0;i<height;i++){
            for(int j=0;j<width;j++){
                ...
                int hSlope=(int)Math.round(Math.toDegrees(slopeHorizontal)*10); //最大900;
                int vSlope=(int)Math.round(Math.toDegrees(slopeVertical)*10); //最大900;
                int total=vSlope*100+hSlope; //最大900900 小于INT.MAX
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            BufferedImage outImage= CalcSlop2Band(image,30);
            ImageIO.write(outImage,"PNG",new FileOutputStream("d:/hainanslope2.png"));

            ReferencedEnvelope re=new ReferencedEnvelope(coverage.getEnvelope2D(),crs);
            GridCoverage2D gc = gcf.create("name", outImage, re);
            GeoTiffWriter geoTiffWriter=new GeoTiffWriter(new File("d:/hainanslope2.tif"));
